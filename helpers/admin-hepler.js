...
await db.get().collection(collections.ADMIN_COLLECTION).insertOne(data)
  ...
  adminLogin: (data)=>{
    return new Promise (async(resolve,reject)=>{
      ...
      let response={}
      let admin=await db.get().collection(collections.ADMIN_COLLECTION).findOne({ email:data.email })
  ...
  getallusers:()=>{
    return new Promise (async(resolve,reject)=>{
      let allusers=await db.get().collection(collections.USER_COLLECTION).find().sort({date:-1}).toArray()
      resolve(allusers)
...
getproductdetails: (proId)=>{
  return new Promise ((resolve,reject)=>{
   db.get().collection(collections.PRODUCT_COLLECTION).findOne(({_id})).then((product)=>{
    console.log(product);
 resolve(product)
})
